# ╔══════════════════════════════════════════════════════════════╗
# ║          RX SMM ZONE — Telegram Bot  (Full Version)         ║
# ║    Colored Buttons · Typing Effect · Admin Panel · Refer    ║
# ╚══════════════════════════════════════════════════════════════╝
...
FREE_OFFER_TEXT  = (
    ...
    "👥 ১ জনকে রেফার করলে পাবেন *১০০ Telegram Members ফ্রী!*\n\n"
...
SERVICES = {
    "tiktok": {
        "name": "⚫ TikTok",
        "items": {
            "tiktok_views":     {"name": "👁 TikTok Views",      "service_id": "1001", "price_per_1k": 2,  "min": 1000, "max": 1000000, "note": "No Life Time"},
    ...
    "telegram": {
        "name": "🔵 Telegram",
        "items": {
            "tg_views":     {"name": "👁 Telegram Views",    "service_id": "3001", "price_per_1k": 1,  "min": 100, "max": 500000, "note": "Life Time"},
    ...
    "instagram": {
        "name": "🟣 Instagram",
        "items": {
            "ig_views":     {"name": "👁 Instagram Views",     "service_id": "5001", "price_per_1k": 1,  "min": 1000, "max": 1000000, "note": "Life Time"},
...
def _svc_kb(items: dict, back_label: str = "🔙 Back") -> ReplyKeyboardMarkup:
    rows = [[_btn(s["name"], "primary")] for s in items.values()]
    rows.append([_btn(back_label, "danger")])
    return ReplyKeyboardMarkup(rows, resize_keyboard=True)

def tiktok_kb()    -> ReplyKeyboardMarkup: return _svc_kb(SERVICES["tiktok"]["items"])
def facebook_kb()  -> ReplyKeyboardMarkup: return _svc_kb(SERVICES["facebook"]["items"])
def telegram_kb()  -> ReplyKeyboardMarkup: return _svc_kb(SERVICES["telegram"]["items"])
def youtube_kb()   -> ReplyKeyboardMarkup: return _svc_kb(SERVICES["youtube"]["items"])
def instagram_kb() -> ReplyKeyboardMarkup: return _svc_kb(SERVICES["instagram"]["items"])
...
# ══════════════════════════════════════════════════════════════
async def _typing(update: Update, ctx: ContextTypes.DEFAULT_TYPE, delay: float = 0.6) -> None:
    await ctx.bot.send_chat_action(update.effective_chat.id, ChatAction.TYPING)
    await asyncio.sleep(delay)

async def _typing_reply(
    update: Update,
    ...
    **kwargs,
    ...
    await _typing(update, ctx, delay)
...
def create_user(uid: int, uname: str, fname: str, refer_by: int = 0) -> None:
    with sqlite3.connect(DB) as c:
        c.execute(
            "INSERT OR IGNORE INTO users"
            "(user_id,username,full_name,balance,total_orders,total_spent,refer_by,refer_count,joined_at)"
            " VALUES(?,?,?,0,0,0,?,0,?)",
...
def update_balance(uid: int, amt: float) -> None:
    with sqlite3.connect(DB) as c:
        c.execute("UPDATE users SET balance=balance+? WHERE user_id=?", (amt, uid))

def save_order(uid, svc_key, svc_name, link, qty, amt, panel_id) -> int:
    with sqlite3.connect(DB) as c:
        cur = c.execute(
...
def get_recent_orders(uid: int, limit: int = 5):
    with sqlite3.connect(DB) as c:
        return c.execute(
...
def get_all_users_detail():
    with sqlite3.connect(DB) as c:
        return c.execute(
            "SELECT user_id,full_name,username,balance,total_orders,total_spent"
...
def get_stats():
    with sqlite3.connect(DB) as c:
        ...
        r = c.execute("SELECT SUM(amount) FROM orders").fetchone()[0] or 0
        d = c.execute("SELECT SUM(amount) FROM deposits WHERE status='Completed'").fetchone()[0] or 0
    return u, o, r, d

def save_deposit(uid: int, amt: float, phone: str, txn_ref: str) -> None:
    with sqlite3.connect(DB) as c:
        c.execute(
            "INSERT INTO deposits(user_id,amount,phone,txn_ref,status,created_at)"
            " VALUES(?,?,?,?,'Pending',?)",
            (uid, amt, phone, txn_ref, datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
        )

def mark_deposit_done(txn_ref: str):
    with sqlite3.connect(DB) as c:
        row = c.execute(
            "SELECT user_id,amount FROM deposits WHERE txn_ref=?", (txn_ref,)
        ).fetchone()
        if row:
            c.execute("UPDATE deposits SET status='Completed' WHERE txn_ref=?", (txn_ref,))
            c.execute("UPDATE users SET balance=balance+? WHERE user_id=?", (row[1], row[0]))
    return row

def handle_refer(new_uid: int, referrer_uid: int) -> bool:
    with sqlite3.connect(DB) as c:
        row = c.execute("SELECT refer_by FROM users WHERE user_id=?", (new_uid,)).fetchone()
        if row and row[0] != 0:
            return False
        c.execute("UPDATE users SET refer_by=? WHERE user_id=?", (referrer_uid, new_uid))
        c.execute("UPDATE users SET refer_count=refer_count+1 WHERE user_id=?", (referrer_uid,))
    return True

# ══════════════════════════════════════════════════════════════
#  SMM PANEL
# ══════════════════════════════════════════════════════════════
def place_smm_order(sid: str, link: str, qty: int):
    try:
        r = requests.post(
            SMM_PANEL_URL,
...
#  PAYMENT
# ══════════════════════════════════════════════════════════════
def make_payment(uid: int, amt: float, phone: str):
    ...
    for attempt in range(3):
        try:
            resp = requests.post(AUTOPAY_URL, headers=hdrs, data=payload, timeout=30).json()
            url  = (resp.get("payment_url") or resp.get("url")
                    or (resp.get("data") or {}).get("payment_url"))
            return (url, txn_ref, resp) if url else (None, txn_ref, resp)
        except requests.exceptions.Timeout:
            if attempt < 2:
                continue
            return None, txn_ref, "পেমেন্ট সার্ভার সাড়া দিচ্ছে না।"
        except Exception as e:
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if refer_by and refer_by != u.id:
        if handle_refer(u.id, refer_by):
            rqty = int(get_setting("refer_reward_qty", "100"))
            try:
    ...
    if get_setting("bot_active", "1") != "1" and u.id != ADMIN_ID:
        await _typing_reply(update, ctx, "⚠️ বট সাময়িকভাবে বন্ধ আছে। পরে আবার চেষ্টা করুন।")
        return
...
async def cmd_refer_claim(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    await _typing_reply(
        update, ctx,
...
async def cmd_addbalance(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
...
async def cmd_subbalance(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    if update.effective_user.id != ADMIN_ID: return
    try:
        tid = int(ctx.args[0]); amt = float(ctx.args[1])
        update_balance(tid, -amt); nb = get_balance(tid)
...
async def cmd_broadcast(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    msg = " ".join(ctx.args)
    uids = get_all_user_ids(); sent = 0
    for uid in uids:
        try:
            await ctx.bot.send_message(uid, f"📢 *RKX SMM ZONE*\n\n{msg}", parse_mode="Markdown")
            sent += 1
        except: pass
    await update.message.reply_text(f"✅ ব্রডকাস্ট: {sent}/{len(uids)}")
...
async def cmd_setprice(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        svc = find_svc(key)
        if not svc:
            await update.message.reply_text("❌ Service key পাওয়া যায়নি।"); return
        svc["price_per_1k"] = price
        await update.message.reply_text(f"✅ {svc['name']} → {price}৳/১০০০")
    except Exception as e:
        await update.message.reply_text(f"Usage: /setprice <key> <price>\n{e}")

async def cmd_userinfo(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    if update.effective_user.id != ADMIN_ID: return
    try:
        tid = int(ctx.args[0]); uu = get_user(tid)
        if not uu:
            await update.message.reply_text("❌ ইউজার পাওয়া যায়নি।"); return
        await update.message.reply_text(
            f"👤 *User Info*\n━━━━━━━━━━━━\n"
            f"🆔 ID: `{uu[0]}`\n📛 নাম: {uu[2]}\n🔗 @{uu[1] or 'N/A'}\n"
            f"💰 ব্যালেন্স: {uu[3]:.2f}৳\n📦 অর্ডার: {uu[4]}\n"
...
async def cmd_setreferqty(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    if update.effective_user.id != ADMIN_ID: return
    try:
        qty = int(ctx.args[0])
        set_setting("refer_reward_qty", str(qty))
        await update.message.reply_text(f"✅ রেফার রিওয়ার্ড: {qty} members")
    except Exception as e:
        await update.message.reply_text(f"Usage: /setreferqty <qty>\n{e}")

async def cmd_setoffer(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    if update.effective_user.id != ADMIN_ID: return
    if not ctx.args:
        await update.message.reply_text("Usage: /setoffer <text>"); return
    set_setting("free_offer_text", " ".join(ctx.args))
    await update.message.reply_text("✅ অফার টেক্সট আপডেট হয়েছে।")

# ══════════════════════════════════════════════════════════════
#  CALLBACKS
# ══════════════════════════════════════════════════════════════
async def cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    q = update.callback_query
    await q.answer()
    u = q.from_user
    d = q.data
    back_btn = [[InlineKeyboardButton("🔙 Back", callback_data="adm_back")]]
    ...
    if d == "confirm_order":
        await _do_confirm_order(q, ctx); return

    if d == "cancel_order":
        ctx.user_data.clear()
        await q.edit_message_text("❌ অর্ডার বাতিল করা হয়েছে।"); return
    ...
    if d == "adm_stats":
        ...
        await q.edit_message_text("💰 *ব্যালেন্স যোগ*\nইউজার ID দাও:", parse_mode="Markdown")
    elif d == "adm_subbal":
        ctx.user_data["adm_step"] = "subbal_uid"
        await q.edit_message_text("💸 *ব্যালেন্স কমাও*\nইউজার ID দাও:", parse_mode="Markdown")
        ...
        await q.edit_message_text("🔍 ইউজার ID লিখো:")
    elif d == "adm_boton":
...
#  CONFIRM ORDER
# ══════════════════════════════════════════════════════════════
async def _do_confirm_order(q, ctx) -> None:
    u     = q.from_user
...
#  MAIN TEXT HANDLER
# ══════════════════════════════════════════════════════════════
async def txt(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    u     = update.effective_user
    ...
    if get_setting("bot_active", "1") != "1" and u.id != ADMIN_ID:
        await _typing_reply(update, ctx, "⚠️ বট সাময়িকভাবে বন্ধ।"); return
    ...
    if adm and u.id == ADMIN_ID:
        ...
        if adm == "addbal_uid":
            ...
            except: await update.message.reply_text("❌ সঠিক User ID দাও।")
            return

        if adm == "addbal_amt":
            try:
                amt = float(t); tid = ctx.user_data["adm_target"]
                update_balance(tid, amt); nb = get_balance(tid)
                await update.message.reply_text(f"✅ {tid} → +{amt}৳ | নতুন: {nb:.2f}৳")
                try:
                    await ctx.bot.send_message(
                ...
                ctx.user_data.clear()
            except: await update.message.reply_text("❌ সঠিক পরিমাণ দাও।")
            return
        ...
        if adm == "search_uid":
            try:
                uu = get_user(int(t))
                if not uu: await update.message.reply_text("❌ পাওয়া যায়নি।")
                else:
                    await update.message.reply_text(
                        f"👤 {uu[2]} | @{uu[1] or 'N/A'}\n"
                ...
                ctx.user_data.clear()
            except: await update.message.reply_text("❌ সঠিক ID দাও।")
            return
    ...
    if t == "🛒 অর্ডার করুন":
        ...
        await _typing_reply(
            update, ctx,
            f"🏪 *সার্ভিস সিলেক্ট করুন* 👇\n\n💰 ব্যালেন্স: *{get_balance(u.id):.2f}৳*",
            reply_markup=order_cat_kb(), parse_mode="Markdown",
        ); return

    if t == "🔙 মেইন মেনু":
        ctx.user_data.clear()
        await _typing_reply(update, ctx, WELCOME, reply_markup=MAIN_KB, delay=0.3); return
    ...
    if t in CAT_MAP and ctx.user_data.get("in_order"):
        ...
        await _typing_reply(
            update, ctx,
            f"{title} 👇\n\n💰 ব্যালেন্স: *{get_balance(u.id):.2f}৳*",
            reply_markup=kb_fn(), parse_mode="Markdown",
        ); return
    ...
    if t == "🔙 Back":
        ...
        await _typing_reply(
            update, ctx,
            f"🏪 *সার্ভিস সিলেক্ট করুন* 👇\n\n💰 ব্যালেন্স: *{get_balance(u.id):.2f}৳*",
            reply_markup=order_cat_kb(), parse_mode="Markdown", delay=0.3,
        ); return
    ...
    if t == "❌ বাতিল করুন":
        ctx.user_data.clear()
        await _typing_reply(update, ctx, "❌ বাতিল।", reply_markup=MAIN_KB, delay=0.2); return

    # ── Order: link ───────────────────────────────────────────
    if step == "link":
        if not (t.startswith("http://") or t.startswith("https://")):
            await _typing_reply(update, ctx,
                "❌ সঠিক লিংক দাও! http/https দিয়ে শুরু হতে হবে।", delay=0.2); return
        svc = find_svc(ctx.user_data.get("sel", ""))
    ...
    if step == "qty":
        if not t.isdigit():
            await _typing_reply(update, ctx, "❌ শুধু সংখ্যা লিখো!", delay=0.2); return
        qty = int(t)
        svc = find_svc(ctx.user_data.get("sel", ""))
    ...
    if t == "💰 ব্যালেন্স":
        uu = get_user(u.id)
        bal = uu[3] if uu else 0.0; tot = uu[4] if uu else 0; spent = uu[5] if uu else 0.0
    ...
    if t == "📦 অর্ডার স্ট্যাটাস":
        ...
        if not rows:
            await _typing_reply(update, ctx, "📦 এখনো কোনো অর্ডার নেই।"); return
        msg = "📦 *সর্বশেষ ৫টি অর্ডার:*\n━━━━━━━━━━━━━━━\n"
        for r in rows:
            em = "✅" if r[4] == "Completed" else "⏳" if r[4] == "Processing" else "❌"
            msg += (f"🆔 `#{r[0]}` | 🛒 {r[1]}\n"
                    f"🔢 {r[2]:,} | 💸 {r[3]:.2f}৳ | {em} {r[4]}\n"
                    f"📅 {r[5]}\n─────────────\n")
        await _typing_reply(update, ctx, msg, parse_mode="Markdown"); return

    # ── Support ───────────────────────────────────────────────
    if t == "🆘 সাপোর্ট":
        kb = [[InlineKeyboardButton("📩 Contact Support",
    ...
    if t == "🎁 ফ্রী সার্ভিস":
        ...
        await _typing_reply(update, ctx, offer, parse_mode="Markdown"); return

    # ── Deposit: amount ───────────────────────────────────────
    if ds == "amt":
        if not t.isdigit() or int(t) < 10:
            await _typing_reply(update, ctx,
                "❌ সর্বনিম্ন *১০ টাকা* দিতে হবে!", parse_mode="Markdown", delay=0.2); return
    ...
    if ds == "phone":
        ...
        phone = t
        await _typing(update, ctx, delay=0.8)
    ...
    if not step and not ds and not adm and not ctx.user_data.get("in_order"):
        ctx.user_data.clear()
        await _typing_reply(update, ctx, WELCOME, reply_markup=MAIN_KB, delay=0.3)
...
def main() -> None:
    ...
    logging.info("✅  RKX SMM Zone Bot চালু হয়েছে!")
